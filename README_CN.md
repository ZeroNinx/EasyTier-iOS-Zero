...
面向 iOS 15+ rootless 越狱环境的 EasyTier 客户端。本分支将 SwiftUI 越狱 App、EasyTier core 包装层和特权 `easytierd` daemon 打包为一个 rootless `.deb`。
...
请在 rootless 越狱设备上安装生成的 `.deb`。桌面入口作为越狱 App 安装在 `/var/jb/Applications/EasyTier.app`，运行时数据和日志保存在 `/var/jb/var/lib/easytier/`。

安装脚本会在注册越狱 App 前清理旧的 `com.zeroninx` 包、daemon 路径，以及匹配的移动端 App/数据容器。
